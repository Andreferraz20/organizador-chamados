...
function DocumentosLista({
  ...
  const [deleteTarget, setDeleteTarget] = useState<DocumentoInfo | null>(null);
  ...
  async function confirmDelete() {
    ...
    setDeleting(true);
    try {
      setDocs(await api.documentacao.delete(categoria, deleteTarget.arquivo));
  ...
  return (
    <div className="mx-auto flex h-full max-w-2xl flex-col p-8">
          ...
          {docs.map((doc) => (
            ...
            >
              ...
              <button
                ...
                title="Excluir"
                className="ml-1 shrink-0 rounded-md p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-600 dark:text-slate-500 dark:hover:bg-red-950/40 dark:hover:text-red-400"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-4 w-4">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded-lg border border-slate-200 bg-white p-4 shadow-xl dark:border-slate-700 dark:bg-slate-800">
            <h3 className="mb-2 text-sm font-semibold text-slate-800 dark:text-slate-100">Excluir este documento?</h3>
            <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
              Isso apaga "{deleteTarget.nome}" permanentemente. Não é possível desfazer.
            </p>
